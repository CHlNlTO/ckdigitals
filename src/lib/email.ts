...
export async function sendEmail(data: ContactFormData): Promise<EmailResponse> {
  ...
  try {
    await transporter.sendMail({
      ...
      subject: "Algotrimo Client Inquiry",
      text: `New message from: ${data.email}\n\nMessage: ${data.message}`, // Plain text fallback
      html: `
        <!DOCTYPE html>
        <html>
          <head>
                    ...
                    <tr>
                      ...
                      ">
                        <p style="
                          ...
                          color: #64748b;
                          font-size: 14px;
                        ">© ${new Date().getFullYear()} Algotrimo. All rights reserved.</p>
                      </td>
                    </tr>
                  </table>
                </td>
